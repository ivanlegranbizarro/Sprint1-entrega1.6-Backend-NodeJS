...
describe('Comprovem que la classe Animal compleixi amb les expectatives', () => {
  ...
  test('the function crearAnimal should create a new instance of Animal if name of animal is Gos/Gat/Cavall', () => {
...
/*
Omar, sé que els tests del nivell 3 els tinc malament (tot i que les funcions sí que funcionen, les he comprovat molts cops), però te'ls envio igualment per veure si amb les teves correccions m'oriento una mica. Disculpa les molèsties.
*/
...
describe('Comprovem que la funció getEmployeeJSON i getSalaryJSON compleixin amb les expectatives', () => {
  test('should return an object with the employee data', () => {
    const mock = jest.fn();
    ...
    expect(getEmployeeJSON(1)).resolves.toMatchObject({
      id: 1,
      name: 'Linux Torvalds',
    });
  });
  ...
  test('should return an object with the salary data', () => {
    ...
    expect(getSalaryJSON(1)).resolves.toMatchObject({
      id: 1,
      salary: 4000,
    });
  });
  ...
  test('should reject the promise if the employee does not exist', () => {
    ...
    expect(getEmployeeJSON(4)).rejects.toEqual(
      expect.stringContaining("No s'ha trobat l'empleat")
    );
  });
  ...
  test('should reject the promise if the salary does not exist', () => {
    ...
    expect(getSalaryJSON(4)).rejects.toEqual(
      expect.stringContaining("No s'ha trobat el salari")
    );
  });
